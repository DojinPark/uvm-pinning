...
void dijkstraCPU(float *graph, float *h_shortestDistances, int sourceVertex, const int N) {
    ...
    bool *finalizedVertices = (bool *)malloc(N * sizeof(bool));

    // --- Initialize h_shortestDistancesances as infinite and h_shortestDistances as false
    for (int i = 0; i < N; i++) h_shortestDistances[i] = FLT_MAX, finalizedVertices[i] = false;
    ...
    for (int iterCount = 0; iterCount < N - 1; iterCount++) {
        ...
        int currentVertex = minDistance(h_shortestDistances, finalizedVertices, sourceVertex, N);
...
//
    ...
    gpu_timer_set();
    initializeArrays <<<iDivUp(graph->numVertices, BLOCK_SIZE), BLOCK_SIZE >>>(finalizedVertices, h_shortestDistances,
...
//
    int iteration = 0;
    while (!allFinalizedVertices(finalizedVertices, graph->numVertices) && iteration < MAX_ITERATION) {
        ...
        for (int asyncIter = 0; asyncIter < NUM_ASYNCHRONOUS_ITERATIONS; asyncIter++) {
            ...
            Kernel1 <<<iDivUp(graph->numVertices, BLOCK_SIZE), BLOCK_SIZE >>>(graph -> vertexArray, graph -> edgeArray, graph -> weightArray, finalizedVertices, h_shortestDistances,
                                                            d_updatingShortestDistances, graph->numVertices, graph->numEdges);
            //gpuErrchk(cudaPeekAtLastError());
            //gpuErrchk(cudaDeviceSynchronize());
            Kernel2 <<<iDivUp(graph->numVertices, BLOCK_SIZE), BLOCK_SIZE >>>(graph -> vertexArray, graph -> edgeArray, graph -> weightArray, finalizedVertices, h_shortestDistances, d_updatingShortestDistances,
                                                            graph->numVertices);
            //gpuErrchk(cudaPeekAtLastError());
            //gpuErrchk(cudaDeviceSynchronize());
	    iteration++;
...
//
    ...
    gpuErrchk(cudaFree(finalizedVertices));
...
//uvm-pinning
    ...
    size_t numVertices;
    bool pref;
    set_envs(&numVertices, sizeof(float), argv[1], argv[2], &pref, argv[3]);
    numVertices /= (4 + 2*neighborsPerVertex);
//
