...
int setup(int argc, char *argv[])
{
  ...
  float free_space_in_gb = std::stof(argv[3]);
  occupy_gpu_ram(free_space_in_gb);

  ull_t N;
  bool pref;
  parse_args(N, argv[1], sizeof(float), pref, argv[2]);
  const int len_factor = 52;
  N -= N % 208; // aligning to the least common multiple of 16 and 52
  layer_size = N / len_factor;
